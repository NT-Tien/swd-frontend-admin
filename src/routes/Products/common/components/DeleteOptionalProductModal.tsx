...
export default function DeleteOptionalProductModal({ children, productId }: DeleteOptionalProductModal) {
    ...
    const deleteOptionalProductMutation = useMutation({
        ...
        onSuccess: () => {
            setTimeout(() => messageApi.success('Optional product deleted successfully'), 250)
            queryClient.invalidateQueries({
                queryKey: queryOptional_GetByProductId({ productId }).queryKey,
            })
        },
        onError: error => {
            setTimeout(() => messageApi.error('Failed to delete optional product'), 250)
            devLog(error)
        },
        onMutate: () => {
            messageApi.open({
                type: 'loading',
                content: 'Deleting Optional Product',
                key: 'deleting-optional-product',
            })
        },
        onSettled: () => {
            messageApi.destroy('deleting-optional-product')
        },
    })
